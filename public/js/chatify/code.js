  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        const avatar = data.user_avatar 
            ? data.user_avatar 
            : "/uploads/avatar/avatar_default.jpg"; // Đường dẫn ảnh mặc định
        // avatar photo
        $(".messenger-infoView")
          .find(".avatar")
...
$(document).ready(function () {
  ...
  $("body").on("click", ".messenger-list-item", function () {
    ...
    const userId = $(this).data("contact");
    const userName = $(this).find("p").text().trim();
    const userAvatar = $(this).find(".avatar").css("background-image").replace(/^url\(["']?/, '').replace(/["']?\)$/, '');
